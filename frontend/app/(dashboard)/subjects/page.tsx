...
export default function SubjectsPage() {
  ...
  const { data, meta, loading, setSearch, setPage, setPerPage, refresh } = useTable<Subject>({ endpoint: '/subjects' });
  ...
  const openEdit = (item: Subject) => { setEditing(item); reset({ name: item.name, status: item.status }); setModalOpen(true); };

  const onSubmit = async (formData: FormData) => {
    try {
      if (editing) { await api.put(`/subjects/${editing.id}`, formData); toast.success('Berhasil diperbarui'); }
      else { await api.post('/subjects', formData); toast.success('Berhasil ditambahkan'); }
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try { await api.delete(`/subjects/${deleteModal.id}`); toast.success('Berhasil dihapus'); setDeleteModal({ open: false }); refresh(); }
    ...
    { accessorKey: 'status', header: 'Status', cell: ({ row }) => <Badge active={row.original.status} /> },
    {
      header: 'Aksi', cell: ({ row }) => (
        <div className="flex gap-2">
          {hasPermission('subject.update') && <button onClick={() => openEdit(row.original)} className="rounded-lg p-1.5 text-blue-600 hover:bg-blue-50"><Pencil size={15} /></button>}
          {hasPermission('subject.delete') && <button onClick={() => setDeleteModal({ open: true, id: row.original.id })} className="rounded-lg p-1.5 text-red-500 hover:bg-red-50"><Trash2 size={15} /></button>}
        </div>
      ),
    },
  ];
  ...
  return (
    <div className="space-y-5">
      <PageHeader title="Mata Pelajaran" description="Kelola data mata pelajaran" onAdd={openAdd} canAdd={hasPermission('subject.create')} />
      <div className="rounded-2xl bg-white p-5 shadow-sm border border-slate-100">
        <div className="mb-4"><SearchInput value="" onChange={setSearch} placeholder="Cari mata pelajaran..." /></div>
